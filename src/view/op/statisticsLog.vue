    ...
    <div class="gva-search-box">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">
        <el-form-item label="项目ID">
          <el-input v-model="searchInfo.projectId" placeholder="搜索条件" />
        </el-form-item>
...
const getTableData = async() => {
  const table = await getStatisticsLogList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteStatisticsLogByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateStatisticsLogFunc = async(row) => {
    const res = await findStatisticsLog({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteStatisticsLogFunc = async (row) => {
    const res = await deleteStatisticsLog({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
